...
main(int argc, const char **argv) {
  ...
  opt_parse.add_opt("frag-keep", 'f', "keep FRAG: on read name", false, frag_keep);
  ...
  while (in >> mr) {
    sam_rec aln(mr.r.get_name(), //qname
                ...
                255, // qual
                std::to_string(mr.seq.size()) + "M", // CIGAR
                "*", // rnext
                0, // pnext
                0, // tlen
                mr.seq, // seq
    ...
    // by definition, walt seq entries are always T-rich
    aln.add_tag("CV:A:T");
